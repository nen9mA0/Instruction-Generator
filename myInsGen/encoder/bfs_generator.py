...
# Here the traversal will be done by BFS. DFS version is in dfs_generator, which I think is a better way. Because I realize
# BFS will cause path explosion when the constraints are too few.
...
class NTContext(object):
    def __init__(self, prev_ntcontext:Optional['NTContext']=None, init_context:dict=None):
        self.context_nodes = []
        if prev_ntcontext:
